...
def get_fragments_for_changed_atoms(mols, changed_atom_tags, radius = 0, 
    ...
    for mol in mols:
                ...
                if atom.GetSmarts().split(':')[1][:-1] in changed_atom_tags:
                    ...
                    if atom.GetFormalCharge() == 0:
                        ...
                        if ':' in symbol:
                            ...
                            symbol = symbol.replace(']', ';+0]')
                    if ':' in symbol:
                        symbol = symbol.replace(':', ';D{}:'.format(len([bond.GetIdx() for bond in atom.GetBonds()])))
                    else:
                        symbol = symbol.replace(']', ';D{}]'.format(len([bond.GetIdx() for bond in atom.GetBonds()])))

                    if v: print("atom.IsInRing():",atom.IsInRing())
        ...
        for atom in mol.GetAtoms():
            symbol = atom.GetSmarts()
            if ':' not in atom.GetSmarts() and 'H' not in atom.GetSmarts():
                if atom.GetTotalNumHs() == 1: symbol = '[' + symbol + 'H]'
                if atom.GetTotalNumHs() == 2: symbol = '[' + symbol + 'H2]'
                if atom.GetTotalNumHs() == 3: symbol = '[' + symbol + 'H3]'
                if atom.GetTotalNumHs() == 4: symbol = '[' + symbol + 'H4]'
                symbol_replacements.append((atom.GetIdx(), symbol))
...
def mol_list_to_inchi(mols):
    ...
    # inchis = []
    # for mol in mols:
    #     inchis.append(Chem.inchi.MolToInchi(mol))
    inchis = [Chem.inchi.MolToInchi(mol) for mol in mols]
...
def main(vorbose,N = 15):
    ...
    i = -1
    tem_sets = []
    for i, example_doc in REACTION_DB.iterrows():
        i += 1
        if v:
            print('##################################')
            print('###        RXN {}'.format(i))
        ...
        try:
            # Unpack
            reaction_smiles = str(example_doc["Reaction_AtomMap"])
            if v: print("reaction_smiles:",reaction_smiles)
            reactant = Chem.MolFromSmiles(reaction_smiles.split('>>')[0])
            reactant_len = len(reactant.GetAtoms())
            product = Chem.MolFromSmiles(reaction_smiles.split('>>')[1])
            product_len = len(product.GetAtoms())
            Chem.SanitizeMol(reactant)
            Chem.SanitizeMol(product)
            labels = str(example_doc['Reaction_label'])
        ...
        product_inchis_split = Chem.inchi.MolToInchi(product)
        reactant_inchi = Chem.inchi.MolToInchi(reactant)
        if reactant_inchi == product_inchis_split:
            if v:
        ...
        if (reactant == None) or (product == None):
            if v:
        ...
        # Check atom mapping
        # pro_map = sum([a.HasProp('molAtomMapNumber') for a in product.GetAtoms()])
        pro_map = sum([a.HasProp('molAtomMapNumber') for a in product.GetAtoms()])
        pro_atom_len = len(product.GetAtoms())
        reac_map = sum([a.HasProp('molAtomMapNumber') for a in reactant.GetAtoms()])
        reac_atom_len = len(reactant.GetAtoms())

        if pro_atom_len < reac_atom_len:
            temp = reactant
            reactant = product
            product = temp
            change = True
        ...
        changed_atoms, changed_atom_tags, err = get_changed_atoms([reactant], [product])
        if err:
            print('Could not get changed atoms')
            print('RXN_ID: {}'.format(i))
            continue
        ...
        reactant_fragments = get_fragments_for_changed_atoms([reactant], changed_atom_tags,
            radius = radius_re, expansion = [], category = 'reactants')
        # Get fragments for products
        # (WITHOUT matching groups but WITH the addition of reactant fragments)
        product_fragments  = get_fragments_for_changed_atoms([product], changed_atom_tags,
            radius = radius_pro, expansion = expand_changed_atom_tags(changed_atom_tags, reactant_fragments),
            category = 'products')
        if change == True:
            if v: print("change:",change)
        ...
        if correct or DO_NOT_TEST:
            ...
            rxn_canonical = convert_to_retro(rxn_canonical)


            result_all = TRANSFORM_DB_all.append(
                {
        ...
        tem_sets.append(rxn_canonical)

    return TRANSFORM_DB,TRANSFORM_DB_all
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--num', type = int, default = 20000,
                        help = 'Maximum number of records to examine; defaults to 15')
    ...
    TRANSFORM_DB = []
    TRANSFORM_DB_all = []
    data_fid = pd.read_excel(config['database_path'])
    REACTION_DB = pd.DataFrame(columns=['Reaction_label','Reaction_AtomMap','type'])

    for i, line in data_fid.iterrows():
        if i == N:
            break
        label = line['Reaction_label']
        final_type = line['Reaction_type']
        line = line['Reaction_AtomMap'].strip()
        reaction_smiles = line.split('\t')[0]
        reaction_smiles = reaction_smiles.split(' ')[0]
        REACTION_DB.loc[i] = [label,reaction_smiles,final_type]

    TRANSFORM_DB,TRANSFORM_DB_all = main(bool(args.v),N = args.num)

    TRANSFORM_DB_all = pd.DataFrame(TRANSFORM_DB_all)
    TRANSFORM_DB_all['Reaction_label'] = TRANSFORM_DB_all['Reaction_label'].astype(int)
    TRANSFORM_DB_all = pd.merge(TRANSFORM_DB_all,data_fid,on = 'Reaction_label')
    TRANSFORM_DB_all['Template_ID'] = TRANSFORM_DB_all['Reaction_label'] + 347 # Because there are 347 expert rules
    if args.num < 11666:
        TRANSFORM_DB_all.to_excel(config['Templates_path']+f'Templates_For_Training_add_rules_{args.num}.xlsx')
    else:
        TRANSFORM_DB_all.to_excel(config['Templates_path']+f'Templates_For_Training_add_rules.xlsx')
